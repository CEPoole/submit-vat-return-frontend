...
import services.{DateService, VatObligationsService, VatReturnsService, VatSubscriptionService}
  ...
  def show(periodKey: String): Action[AnyContent] = (authPredicate andThen mandationStatusCheck).async { implicit user =>
    ...
    user.session.get(SessionKeys.returnData) match {
      case Some(model) => {
        ...
        vatObligationsService.getObligations(user.vrn) flatMap {
          case Right(obs) => {
            val ob = obs.obligations.head
            vatSubscriptionService.getCustomerDetails(user.vrn) map {
              case Right(customerDetails) => {
                val viewModel = ConfirmSubmissionViewModel(sessionData, periodKey, customerDetails.clientName, ob, customerDetails.hasFlatRateScheme)
                Ok(views.html.confirm_submission(viewModel, user.isAgent))
              }
              case _ => {
                val viewModel = ConfirmSubmissionViewModel(sessionData, periodKey, None, ob, flatRateScheme = false)
                Ok(views.html.confirm_submission(viewModel, user.isAgent))
              }
            }
          }
              ...
              case Right(obs) =>
                if(dateService.dateHasPassed(obs.obligations.head.end)) {
                  ...
                  vatReturnsService.submitVatReturn(user.vrn, submissionModel) map {
                    case Right(_) =>
                      auditService.audit(
                        SubmitVatReturnAuditModel(user, model, periodKey, obs.obligations.head),
